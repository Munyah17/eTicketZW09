...
// Shared shell for /admin and /super-admin — same grouped sidebar, top bar,
// and mobile menu, differing only in which links/labels/role each caller
// passes in. Keeping this in one place is what lets Admin and Super Admin
// look and behave identically (per user request: "one portal", not two
// differently-styled dashboards) while still being genuinely separate
// route trees with their own role gating in each layout.
export function AdminShell({
  ...
  const NavLinks = ({ onNav }: { onNav?: () => void }) => {
    const groups = links.reduce<Record<string, AdminShellLink[]>>((acc, link) => {
        ...
        {Object.entries(groups).map(([group, groupLinks]) => (
          <div key={group} className="mb-2">
            ...
            {groupLinks.map((link) => {
              ...
              const hasBadge = payoutsHref && link.href === payoutsHref && pendingPayouts > 0;
              return (
                <Link
                  key={link.href}
                  href={link.href}
        ...
        <div className="hidden lg:flex h-14 items-center justify-between px-8 bg-white dark:bg-card border-b sticky top-0 z-30">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <span className="font-medium text-foreground">
              {links.find((l) => isLinkActive(l.href))?.label || "Dashboard"}
            </span>
          </div>
