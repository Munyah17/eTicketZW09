...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    loadStats();
    window.addEventListener("eticket:events-updated", loadStats);
    return () => window.removeEventListener("eticket:events-updated", loadStats);
  }, [loadStats]);

  const { grossRevenue, platformFees, totalTicketsSold, publishedEvents, totalEvents, organizerCount } = stats;
  ...
  const kpis = [
    ...
    },
    {
      ...
      sub: `Across ${publishedEvents} live event${publishedEvents !== 1 ? "s" : ""}`,
      icon: Ticket,
      accent: "bg-blue-500",
    ...
    },
    {
      ...
      sub: `${totalEvents} total on platform`,
      icon: Calendar,
    ...
    },
    {
      ...
      icon: CreditCard,
      accent: "bg-slate-400",
      light: "bg-slate-50 text-slate-600 dark:bg-slate-800 dark:text-slate-400",
    },
    {
      title: "Platform Fee",
          ...
          <CardContent className="p-0">
            {stats.recentEvents.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center px-6">
                ...
                <p className="font-medium text-sm">{loading ? "Loading…" : "No events yet"}</p>
                {!loading && (
              ...
              <div className="divide-y">
                {stats.recentEvents.map((event, i) => {
                  const pct = event.total_tickets > 0
                    ? Math.round((event.sold_tickets / event.total_tickets) * 100)
                    : 0;
                  return (
                    <div key={event.id} className="flex items-center gap-4 px-6 py-3.5 hover:bg-muted/40 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground truncate">
                          ...
                          {new Date(event.date).toLocaleDateString("en-ZW", { day: "numeric", month: "short" })}
                        </p>
                      </div>
        ...
        <CardContent className="p-0">
          {stats.recentOrganizers.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center px-6">
              ...
              <p className="font-medium text-sm">{loading ? "Loading…" : "No organizers yet"}</p>
              {!loading && (
                <p className="text-xs text-muted-foreground mt-1">
