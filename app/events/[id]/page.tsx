...
export default function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // TypeScript guard - notFound() throws so event is defined below
  const safeEvent = event!;

  const handleTicketSelect = (ticketTypeId: string) => {
    setSelectedTicketType(ticketTypeId);
    // Scroll to the purchase form on mobile
  ...
  const categoryLabel = EVENT_CATEGORIES.find((c) => c.value === safeEvent.category)?.label || safeEvent.category;
  const availableTickets = safeEvent.totalTickets - safeEvent.soldTickets;
  const soldPercentage = Math.round((safeEvent.soldTickets / safeEvent.totalTickets) * 100);
              ...
              <div className="lg:col-span-1">
                <Card>
                  <CardContent className="p-6">
                    ...
                    <h1 className="text-2xl font-bold leading-tight text-balance">{ev.title}</h1>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {ev.platformNegotiated?.isPlatformEvent
                        ? `Organized by E-TicketsZW on behalf of ${ev.platformNegotiated.originalOrganizerName}`
                        : `by ${ev.organizerName}`
                      }
                    </p>
                    {ev.organizerCategory && (
                      <p className="mt-1 text-xs text-primary">
                        {ev.organizerCategory}
                        {ev.organizerSubtype && ` · ${ev.organizerSubtype}`}
                      </p>
                    )}
                    ...
                    <div className="mt-6 space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium">{formatDate(ev.date)}</p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium">{formatTime(ev.time)}</p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{ev.city}</p>
                        </div>
                      </div>
                    </div>
                  ...
                  {ev.ticketTypes.map((ticketType) => {
                    ...
                    return (
                      <Card
                        ...
                        onClick={() => {
                          if (available > 0) handleTicketSelect(ticketType.id);
                        }}
                      >
                ...
                <Card className="mt-4">
                  <CardContent className="p-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h3 className="font-semibold">{ev.venue}</h3>
                        <p className="text-sm text-muted-foreground">{ev.city}, Zimbabwe</p>
